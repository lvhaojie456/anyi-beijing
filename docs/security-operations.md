...
- 管理员操作记录：`GET /admin/audit-logs` 可查看最近 200 条，支持按操作类型和操作者筛选。
- 上传内容审核：所有上传都会写入 `upload_reviews`；文本做关键词初筛，图片和音频进入人工待审。后台在上传审核队列里直接预览图片、音频和文本，并显示用途与上传者。
...
1. 管理后台页面：用管理员账号登录后台，在「文件删除」队列点击「处理待删除文件」。仍被头像、背景、语音或形象引用的文件会标记为失败并保留，解除引用后在详情里「重新加入待删除」。
...
- 账号注销已可在后台完成（`DELETE /admin/users/:id`，需输入用户名确认）：注销会清理该用户的全部私有数据，并把上传文件写入删除队列。
