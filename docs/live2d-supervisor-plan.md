...
第三个问题的关键事实（实测修正）：`input/src_img.png` 的 alpha 覆盖 66.7% 画布只是 1024×1536 在正方形画布里的信箱区域，不能作为泄漏依据。真正的机制是 See-through 把**纯白背景**当作人物并入衣服图层：同一张图换成灰底重拆，`topwear` 从占画布 79% 回到 10%；逐层深度图里泄漏背景深度饱和 ≥ 250，`depth < 250` 就能切出真正的上衣。因此补救有两道：进拆层前把透明或近白背景填成中性灰（预防），拆层后按深度图与遮罩裁剪（兜底）。
