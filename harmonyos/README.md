...
| AI 陪伴 | 爷爷/奶奶双角色、本地原生聊天界面、云端 AI 回复、2D 数字人 ArkWeb 页面 |
...
AI 长期记忆默认关闭。用户在陪伴页开启后，聊天记录和明确表达的偏好、重要日子会按角色保存到云端；Android 端提供记忆查看和删除入口，鸿蒙端提供开关并共用同一套云端数据。
...
2. 在 DevEco Studio 中选择 **Open Project**，打开本 `harmonyos` 目录。
